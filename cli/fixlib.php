...
function page_format_redraw_sections($course) {
    ...
    if ($oldsections) {
        mtrace("Registering old sections.");
    ...
    if ($pages) {
        ...
        foreach ($pages as $p) {
            ...
            if ($pis) {
                foreach ($pis as $pi) {
                    if (!array_key_exists($pi->cmid, $coursecmids)) {
                        mtrace("Page item to a module ($pi->cmid) NOT in course ($course->id) ");
                        continue;
                    }
                    $sequence[] = $pi->cmid;
            ...
            mtrace("Section {$newid}/{$section->section} recorded.");

            $i++;
        }
    } else {
        mtrace("No pages found");
    }
    ...
    mtrace("Section {$section0->id}/{$section0->section} recorded.");
    ...
    mtrace("Section modules remaped in section 0.");
}